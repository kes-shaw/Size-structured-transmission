...
custom_order <- c("Uniform Small", "Uniform Medium", "Uniform Large", "Equal", "Small Skewed")
dataset <- expand.grid(Structure = factor(custom_order, levels = custom_order),
                       P_Level = 0:283)


dataset <- dataset %>%
  mutate(Structure = factor(Structure, levels = custom_order)) %>%
  arrange(Structure)

dataset=dataset%>%
  mutate(N_S=rep(c(18,0,0,6,12), each=284))%>%
  mutate(N_M=rep(c(0,18,0,6,3), each=284))%>%
  mutate(N_L=rep(c(0,0,18,6,3), each=284))
...
small_preds = mapply(FUN=prev_S, P0 =dataset_small[,"P_Level"]/18, N_S=dataset_small[,"N_S"]/15, N_M=dataset_small[,"N_M"]/15, N_L=dataset_small[,"N_L"]/15,
                     MoreArgs = list(t=1, sigma_0 = 0.6085019, epsilon_0 = 0.2498385, sigma_sz = -0.2139004, epsilon_sz = 0.1835933))

small_predictions = cbind(dataset_small,small_preds)
colnames(small_predictions)=c("Structure", "P0", "N_S", "N_M", "N_L", "Prevalence")
...
med_preds = mapply(FUN=prev_M, P0 =dataset_med[,"P_Level"]/18, N_S=dataset_med[,"N_S"]/15, N_M=dataset_med[,"N_M"]/15, N_L=dataset_med[,"N_L"]/15,
                     MoreArgs = list(t=1, sigma_0 = 0.6895559, epsilon_0 = 0.1904044, sigma_sz = -0.2282718, epsilon_sz = 0.1950911))


med_predictions = cbind(dataset_med, med_preds)
colnames(med_predictions)=c("Structure", "P0", "N_S", "N_M", "N_L", "Prevalence")
...
lg_preds = mapply(FUN=prev_L, P0 =dataset_lg[,"P_Level"]/18, N_S=dataset_lg[,"N_S"]/15, N_M=dataset_lg[,"N_M"]/15, N_L=dataset_lg[,"N_L"]/15,
                   MoreArgs = list(t=1, sigma_0 = 0.6895559, epsilon_0 = 0.1904044, sigma_sz = -0.2282718, epsilon_sz = 0.1950911))


lg_predictions = cbind(dataset_lg, lg_preds)
colnames(lg_predictions)=c("Structure", "P0", "N_S", "N_M", "N_L", "Prevalence")
...
sm=ggplot(data=prev_small, mapping=aes(x=P_Level*18, y=Prevalence.mean, colour = factor(Structure)))+
  ...
  geom_errorbar(data=prev_small,mapping=aes(ymin=Prevalence.mean-(Prevalence.sd/4), ymax=Prevalence.mean+(Prevalence.sd/4)), width=.05,
                position=position_dodge(.9), colour = "black")+
  scale_colour_discrete(name = "Size Structure", type = c("black","#007849","#0375B4"))+
  labs(x = "", y = "", title = "A. Small snails")+
  scale_x_continuous(breaks=c(0,50,100,150,200,250))+
  theme_classic(base_size=16)

a=sm+geom_line(data=small_predictions, aes(x=P0, y=Prevalence, color=Structure), lwd=1)
a
...
med=ggplot(data=prev_med, mapping=aes(x=P_Level*18, y=Prevalence.mean, colour = factor(Structure)))+
  ...
  geom_errorbar(data=prev_med,mapping=aes(ymin=Prevalence.mean-(Prevalence.sd/4), ymax=Prevalence.mean+(Prevalence.sd/4)), width=.05,
                position=position_dodge(.9), colour = "black")+
  scale_colour_discrete(name = "", type = c("black","#007849","#EB6E80"))+
  labs(x = "", y = "Prevalence ± SE", title = "B. Medium snails")+
  scale_x_continuous(breaks=c(0,50,100,150,200,250))+
  theme_classic(base_size=16)

b=med+geom_line(data=med_predictions, aes(x=P0, y=Prevalence, color=Structure), lwd=1)
b
...
lg=ggplot(prev_lg, mapping=aes(x=P_Level*18, y=Prevalence.mean, colour = factor(Structure)))+
  ...
  geom_errorbar(data=prev_lg,mapping=aes(ymin=Prevalence.mean-(Prevalence.sd/4), ymax=Prevalence.mean+(Prevalence.sd/4)), width=.05,
                position=position_dodge(.9), colour = "black")+
  scale_colour_discrete(name = "", type = c("black", "#007849","#590494"))+
  labs(x = "Parasite density, L-1", y = "", title = "C. Large snails")+
  scale_x_continuous(breaks=c(0,50,100,150,200,250))+
  theme_classic(base_size=16)

c=lg+geom_line(data=lg_predictions, aes(x=P0, y=Prevalence), lwd=1)
